...
GT_PATH = os.path.join(parent_path, 'input','detection_results_yolo_info')
#print(GT_PATH)
os.chdir(GT_PATH)
...
for tmp_file in txt_list:
  ...
  for fname in os.listdir('../images_optional_info'):
    if fname.startswith(image_name):
      ## image found
      #print(fname)
      img = cv2.imread('../images_optional_info/' + fname)
